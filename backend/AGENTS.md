...
- `pyproject.toml` - uv project. Runtime deps: fastapi, uvicorn, itsdangerous, openai. Dev: pytest, httpx2. Pytest config sets `pythonpath = ["."]` and deselects `live` tests by default
...
- `app/auth.py` - hardcoded `user`/`password` check, signed session cookie via itsdangerous (`SECRET_KEY` env, dev default), `CurrentUser` dependency
